...
const createColumns = ({ editList, clickDel }) => {
  return [
    ...
    //   key: "origin_price",
    //   render(row) {
    //     return h("span", filter.currency(row.origin_price));
    //   },
    // },
    // {
    //   title: "Price",
    //   key: "price",
    //   render(row) {
    //     return h("span", filter.currency(row.price));
    //   },
    // },
    {
      title: "Enable",
...
<template>
  <main class="bg-creamWhite2 h-screen w-full overflow-y-auto text-text_color">
    <div class="sticky top-0 z-10 border-b border-solid border-gray-300 p-5 text-lg font-medium bg-white">產品管理</div>
